...
# 1. Միասնական ֆունկցիա ֆայլերի պահպանման համար
...
class BloodGroup(models.Model):
    group_name = models.CharField(max_length=11, unique=True, verbose_name="Խմբի անվանում")
...
class CustomUser(AbstractUser):
    ...
    profile_picture = models.ImageField(upload_to=get_face_image_path, null=True, blank=True, verbose_name="Պրոֆիլի նկար")
...
class UserFaceImage(models.Model):
    user = models.ForeignKey(CustomUser, on_delete=models.CASCADE, related_name='face_images', verbose_name="Օգտատեր")
    image = models.ImageField(upload_to=get_face_image_path, verbose_name="Ճանաչման նկար")
    uploaded_at = models.DateTimeField(auto_now_add=True, verbose_name="Վերբեռնման ամսաթիվ")
    class Meta: verbose_name="Դեմքի ճանաչման նկար"; verbose_name_plural="Դեմքի ճանաչման նկարներ"; ordering=['-uploaded_at']
    def __str__(self): return f"Նկար {self.user.username}-ի համար"
...
class DoctorProfile(models.Model):
    ...
    def __str__(self): return f"Բժիշկ՝ {self.user}"
...
class PatientProfile(models.Model):
    ...
    def __str__(self): return f"Պացիենտ՝ {self.user}"
# 4. Միջանկյալ (Pivot) մոդելներ
# -----------------------------------------------------------------------------
