...
def register_view(request):
    ...
    try:
        ...
        messages.success(
            request, "Դուք հաջողությամբ գրանցվել եք։ Այժմ կարող եք մուտք գործել։"
        )
        ...
        messages.error(
            request,
            f"Տվյալների սխալ։ Խնդրում ենք ստուգել լրացված դաշտերը։ {e.messages[0]}",
        )
        return render(request, "register.html", context)
...
def qr_code_view(request):
    ...
    context = {"qr_image_data_uri": data_uri, "profile_url": profile_url}
